...
- **Production upload fix** (v1.0.2): native uploads switched from fetch+FormData to `expo-file-system/legacy` uploadAsync (MULTIPART) — fetch FormData silently failed in release APKs; all 9 upload call sites now show visible error toasts; system crop screen removed (`allowsEditing: false`) because it had no confirm button on Android; edit-profile cold-mount prefill fixed; PUT /users/me now supports clearing fields (exclude_unset); backend storage_helper priority = own S3/R2 → own LOCAL_STORAGE_DIR disk → Emergent (preview only). app.json: version 1.0.2, versionCode 2, package com.glinttechnologies.glint, productionApiUrl → user's Render backend. Verified by testing agent iterations 4-5 (13/13 backend upload tests + all web flows).
...
- **5 signature features** (2026-06): (1) Golden Hours — daily 18:00-19:00 UTC window, golden posts get gold aura + Golden Feed (`/golden`), home countdown banner; (2) Glint Spark — spark tokens (50 start, +10 daily), tip posts 🪙, balance on profile; (3) Voice Stories — record voice drops with gold waveform viewer + audience; (4) Inner Circle — max 10 close friends, audience=inner posts/stories, gold star ring in story bar, manage screen; (5) AI Caption Studio — Emergent LLM (gpt-5.4-mini) generates tone-based captions in composer. Verified: 45/45 backend tests + all frontend flows pass.
